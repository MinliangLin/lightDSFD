...
parser.add_argument("--path", default=None, type=str, help="glob pattern of input")
parser.add_argument("--save", type=str, help='output path')
    ...
    for f in files:
        ...
        if len(keep_idx) > 0:
            ...
            for box, s in zip(keep_boxes, keep_scores):
                ...
                cv2.rectangle(img_cv2, (box[0], box[1]-5), (box[0]+50, box[1]+5), color=(0, 255, 0), thickness=-1) 
                cv2.putText(img_cv2, "{:.2f}".format(s), (box[0], box[1]+5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color=(255,255,255), thickness=2)
